        ...
        if (event?.request?.userAttributes?.sub) {
            ...
            const params = {
                Item: {
                    ...
                    'createdAt': { S: date },
                    'updatedAt': { S: date },
                    '__typename': event?.request?.userAttributes["custom:provider"].toString() === "1" ? { S: 'Provider' } : { S: 'User' }
                },
                TableName: event?.request?.userAttributes["custom:provider"].toString() === "1" ? process.env.PROVIDERTABLE : process.env.USERTABLE
            };
